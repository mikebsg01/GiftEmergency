...
if (! function_exists('url')) {
  function url(string $url = '') {
    $base_url = env('APP_URL', realpath(dirname(dirname(__FILE__))));
    $url = startsWith($url, DIRECTORY_SEPARATOR) ? substr($url, 1) : $url;
    return "$base_url/$url";
  }
}

if (! function_exists('base')) {
  function base(string $uri = '') {
    $base = realpath(dirname(dirname(__FILE__)));
    $uri = startsWith($uri, DIRECTORY_SEPARATOR) ? substr($uri, 1) : $uri;
    return realpath("$base/$uri");
  }
}
...
if (! function_exists('isAdmin')) {
  function isAdmin() {
    $user_data = getUserData(['is_admin']);
